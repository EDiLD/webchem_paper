...
%DIF DEL submission/article.tex   Sat Apr 23 21:36:06 2016
%DIF ADD revision1/article.tex    Sat Apr 23 22:57:50 2016
...
%DIF 25c25
%DIF < \Plaintitle{webchem: an R Package to retrieve Chemical Information from the Web}
%DIF -------
\Plaintitle{webchem: an R Package to Retrieve Chemical Information from the Web} %DIF > 
%DIF -------
\Shorttitle{\pkg{webchem}: Chemical Information from the Web}

\Abstract{
A lot of chemical information is freely available online, including identifiers, experimental and predicted chemical properties.
...
To simplify, robustify and automate this task, i.e.\DIFaddbegin \ \DIFaddend to search and retrieve chemical information from the web, we created the \pkg{webchem} package for the free and open source \proglang{R} language \citep{r_2015, Wehrens_2011}.
...
We describe the basic functionality of the package and demonstrate with a few use cases how to clean and retrieve new data with \pkg{webchem}.
...
The development repository is hosted on GitHub (\url{https://github.com/ropensci/webchem}) and a stable version is released on the official \proglang{R} repository (CRAN, \DIFdelbegin %DIFDELCMD < \url{https://goo.gl/OKbZaF}%%%
...
They follow the format of \code{source\_function}, e.g.\DIFaddbegin \ \DIFaddend \code{cs\_compinfo} uses ChemSpider as source (see next section) to retrieve compound information.
...
The prefix \code{get} is used to denote these functions, e.g.\DIFaddbegin \ \DIFaddend \code{get\_csid} to retrieve the identifier used in ChemSpider.
...
\begin{description}
  ...
  \item[Wikidata \citep{wiki}]{Wikipedia contains information entries for over 15,000 chemicals \citep{Ertl_Patiny_Sander_Rufener_Zasso_2015}. Currently \pkg{webchem} can query only chemical identifiers.}
...
\begin{figure}[ht]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics{fig1.pdf}
%DIFDELCMD <   %%%
...
\DIFdelbegin %DIFDELCMD < \section[Use Cases]{%%%
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> install.packages("webchem")
%DIFDELCMD < R> library(webchem)
%DIFDELCMD < \end{CodeInput}
...
(i) \code{jagst}: This dataset comprises environmental monitoring data of organic substances in the river Jagst of the year 2013, Germany.
...
\DIFaddbegin \DIFadd{First we load the dataset and take a look at the first six rows:
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # load jagst dataset
%DIFDELCMD < R> data(jagst)
%DIFDELCMD < R> # print first 6 lines of data
%DIFDELCMD < R> head(jagst)
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD <         date          substance value qual
%DIFDELCMD < 1 2013-01-04 2,4-Dimethylphenol 0.006    <
%DIFDELCMD < 2 2013-01-29 2,4-Dimethylphenol 0.006    <
%DIFDELCMD < 3 2013-02-26 2,4-Dimethylphenol 0.006    <
%DIFDELCMD < 4 2013-03-26 2,4-Dimethylphenol 0.006    <
%DIFDELCMD < 5 2013-04-23 2,4-Dimethylphenol 0.006    <
%DIFDELCMD < 6 2013-05-22 2,4-Dimethylphenol 0.006    <
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
\begin{CodeInput}
...
(ii) \code{lc50}: This data consists of median acute lethal concentration for the water flea daphnia magna in 48 h tests ($LC_{50, D.magna, 48h}$) of 124 insecticides.
The data has been retrieved from the EPA ECOTOX database \citep{epa_2016}.

\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> data(lc50)
%DIFDELCMD < R> head(lc50)
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD <        cas        value
%DIFDELCMD < 4  50-29-3    12.415277
%DIFDELCMD < 12 52-68-6     1.282980
%DIFDELCMD < 15 55-38-9    12.168138
%DIFDELCMD < 18 56-23-5 35000.000000
%DIFDELCMD < 21 56-38-2     1.539119
%DIFDELCMD < 36 57-74-9    98.400000
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
...
\DIFdelbegin %DIFDELCMD < \subsection[Query Identifiers]{%%%
...
First we search for ETOX internal ID numbers \DIFaddbegin \DIFadd{using the substance names}\DIFaddend :

\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # unique substance names
%DIFDELCMD < R> subs <- unique(jagst$substance)
%DIFDELCMD < R> # search ETOX IDs
%DIFDELCMD < R> ids <- get_etoxid(subs, match = 'best')
%DIFDELCMD < R> head(ids)
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD <   etoxid                           match distance                  query
%DIFDELCMD < 1   8668     2,4-Dimethylphenol ( 8668 )        0     2,4-Dimethylphenol
%DIFDELCMD < 2   8494 4-Chlor-2-methylphenol ( 8494 )        0 4-Chlor-2-methylphenol
%DIFDELCMD < 3   <NA>                            <NA>     <NA>     4-para-nonylphenol
%DIFDELCMD < 4   8397                Atrazin ( 8397 )        0                Atrazin
%DIFDELCMD < 5   7240                 Benzol ( 7240 )        0                 Benzol
%DIFDELCMD < 6   7331        Desethylatrazin ( 7331 )        0        Desethylatrazin
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
...
\pkg{webchem} always returns a named list (one entry for each substance) and the available output / information content can be very voluminous.
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # query SMILES from PubChem
%DIFDELCMD < R> cids <- get_cid(etox_cas)
%DIFDELCMD < R> pc_data <- pc_prop(cids, properties = c('CanonicalSMILES'))
%DIFDELCMD < R> pc_smiles <- smiles(pc_data)
%DIFDELCMD < R> 
%DIFDELCMD < R> # query InChiKey from ChemSpider
%DIFDELCMD < R> # token <- '<yourtokenhere>'
%DIFDELCMD < R> csids <- get_csid(etox_cas, token = token)
%DIFDELCMD < R> cs_data <- cs_compinfo(csids, token = token)
%DIFDELCMD < R> cs_inchikey <- inchikey(cs_data)
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # combine in one data.frame
%DIFDELCMD < R> res <- data.frame(name = subs, cas = etox_cas, smiles = pc_smiles, 
%DIFDELCMD < +                    cid = pc_data$CID, inchikey = cs_inchikey, 
%DIFDELCMD < +                    csid = cs_data$csid, 
%DIFDELCMD < +                    stringsAsFactors = FALSE)
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{or ChemSpider
}\begin{CodeChunk}
\begin{CodeInput}
...
\DIFaddbegin \DIFadd{Finally, we combine the queried data into one data.frame
}\begin{CodeChunk}
\begin{CodeInput}
...
\DIFaddend Note, that in order to use the ChemSpider functions, a personal authentication key (\code{token}) is needed, which can be retrieved from the ChemSpider webpage. 
...
%DIFDELCMD < % latex table generated in R 3.2.3 by xtable 1.8-2 package
%DIFDELCMD < % Fri Mar  4 08:35:28 2016
%DIFDELCMD < \begin{tabular}{llllll}
%DIFDELCMD <   \toprule
%DIFDELCMD < Name & CAS & SMILES\textsuperscript{a} & CID & InChIKey\textsuperscript{a} & CSID \\ 
%DIFDELCMD <   \midrule
%DIFDELCMD < 2,4-Dimethylphenol & 105-67-9 & CC1=CC(... & 7771 & KUFFULV... & 13839123 \\ 
%DIFDELCMD <   4-Chlor-2-methylphenol & 1570-64-5 & CC1=C(C... & 14855 & RHPUJHQ... & 14165 \\ 
%DIFDELCMD <   4-para-nonylphenol & - & - & - & - & - \\ 
%DIFDELCMD <   Atrazin & 1912-24-9 & CCNC1=N... & 2256 & MXWJVTO... & 2169 \\ 
%DIFDELCMD <   Benzol & 71-43-2 & C1=CC=C... & 241 & UHOVQNZ... & 236 \\ 
%DIFDELCMD <   Desethylatrazin & 6190-65-4 & CC(C)NC... & 22563 & DFWFIQK... & 21157 \\ 
...
\end{tabular}
\end{CodeChunk}
\DIFaddendFL \caption{Identifiers for the jagst datasets as queried with \pkg{webchem}. Only the first 6 entries are shown. \DIFdelbeginFL \DIFdelFL{- = not found. \textsuperscript{a} Only }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{For SMILES and InChIKey only }\DIFaddendFL the first 7 characters are shown. \DIFaddbeginFL \DIFaddFL{- = not found.}\DIFaddendFL }
\label{tab:comptable}
\end{table}
...
The Compendium of Pesticide Common Names \citep{wood} contains such information and can be easily queried \DIFaddbegin \DIFadd{using CAS numbers }\DIFaddend with \pkg{webchem}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{: 
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # query the compendium using CAS-numbers
%DIFDELCMD < R> aw_data <- aw_query(lc50$cas, type = 'cas')
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # shows internal structure of the data-object
%DIFDELCMD < R> # str(aw_data[[1]])
%DIFDELCMD < R> # extract chemical group
%DIFDELCMD < R> igroup <- sapply(aw_data, function(y) y$subactivity[1])
%DIFDELCMD < R> igroup[1:3]
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD <                                   50-29-3 
%DIFDELCMD <             "organochlorine insecticides" 
%DIFDELCMD <                                   52-68-6 
%DIFDELCMD <                "phosphonate insecticides" 
%DIFDELCMD <                                   55-38-9 
%DIFDELCMD < "phenyl organothiophosphate insecticides" 
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # query PHYSPROP DATABASE
%DIFDELCMD < R> pp_data <- pp_query(lc50$cas)
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{CodeChunk}
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # look  at internal structure
%DIFDELCMD < R> # str(pp_data[[1]])
%DIFDELCMD < R> lc50$logp <- sapply(pp_data, function(y){
%DIFDELCMD < +    if (length(y) == 1 && is.na(y))
%DIFDELCMD < +      return(NA)
%DIFDELCMD < +    y$prop$value[y$prop$variable == 'Log P (octanol-water)']
%DIFDELCMD < +  })
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \end{CodeChunk}
%DIFDELCMD < %%%
...
\pkg{webchem} provides also basic functions to check identifiers that can be used for data quality assessment.
...
\DIFaddend or web resources \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{like ChemSpider
}\begin{CodeChunk}
\begin{CodeInput}
...
\DIFdelbegin %DIFDELCMD < \begin{CodeChunk}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # simple formatting check
%DIFDELCMD < R> is.inchikey('BQJCRHHNABKAKU-KBQPJGBKS-AN')
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD < Hyphens not at position 15 and 26.
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD < [1] FALSE
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # or using ChemSpider
%DIFDELCMD < R> is.inchikey('BQJCRHHNABKAKU-KBQPJGBKSA-5', 
%DIFDELCMD < +        type = 'chemspider')
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD < [1] FALSE
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \begin{CodeInput}
%DIFDELCMD < R> # formatting check
%DIFDELCMD < R> is.cas('64-17-6')
%DIFDELCMD < \end{CodeInput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD < Checksum is not correct! 5 vs. 6
%DIFDELCMD < \end{CodeOutput}
%DIFDELCMD < \begin{CodeOutput}
%DIFDELCMD < [1] FALSE
%DIFDELCMD < \end{CodeOutput}
...
%DIFDELCMD < %%%
\DIFdelend \section[Discussion]{Discussion}
...
Within the \proglang{Python} ecosystem \DIFdelbegin \DIFdel{(a popular programming language) }\DIFdelend the libraries \pkg{PubChempy} \citep{pubchempy}, \pkg{ChemSpiPy} \citep{chemspipy} and \pkg{CIRpy} \citep{cirpy} are available for similar tasks as those outlined here.
\pkg{webchem} is not specialized and tries to integrate many data sources and for some of these provides a unique programmatic interface.
...
\pkg{webchem} provides an easy way to create such meta tables like Table \ref{tab:comptable}.
...
\DIFdelbegin %DIFDELCMD < \bibliography{references.bib}
%DIFDELCMD < %%%
