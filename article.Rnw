...
It facilitates data cleaning, identification and reporting of substances and retrieval of chemical data.
Consequently it reduces the time researchers spend on chemical data compilation.
}
...
They follow the format of \code{source\_function}, e.g. \code{cs\_compinfo} uses ChemSpider as source (see next section) to retrieve compound informations.
...
Overall, it took only 5 \proglang{R} statements to retrieve the needed data, clean and plot it (using \pkg{ggplot2} \citep{ggplot2}).
